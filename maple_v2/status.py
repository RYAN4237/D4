...
def extract_status_bars_precise(img):
    """Extract HP and MP bar regions and return (hp_region, mp_region, status_region, (x_offset,y_offset))."""
    h, w = img.shape[:2]
    ...
    hp_region = status_region[:, 140:280]
    mp_region = status_region[:, 290:420]
...
def calculate_bar_fill_ratio(bar_region, color_type='red'):
    """Calculate fill percentage of HP/MP bars using color masks.
    Returns: (percentage, mask, bar_start, bar_end)
    """
    ...
    if color_type == 'red':
        ...
        mask1 = cv2.inRange(b, 100, 255)
        mask2 = cv2.inRange(r, 0, 200)
        mask3 = cv2.inRange(g, 0, 200)

        b_minus_r = cv2.subtract(b, r)
        b_minus_g = cv2.subtract(b, g)

        mask4 = cv2.inRange(b_minus_r, 30, 255)
        mask5 = cv2.inRange(b_minus_g, 20, 255)
    ...
    bar_start = int(np.min(active_cols))
    bar_end = int(np.max(active_cols))
    bar_width = int(bar_end) - int(bar_start) + 1
    full_bar_width = int(w)
